...
class ReplayBuffer(object):
    ...
    def update_rewards(self, rewards, idxes):
        ...
        """
        assert type(rewards) == np.ndarray or type(rewards) == list
    ...
    def _encode_sample(self, idxes):
        ...
        for i in idxes:
            ...
            dones.append(done)
        return np.array(obses_t), np.array(actions), np.array(rewards), np.array(obses_tp1), np.array(dones), idxes
    ...
    def return_all(self):
        return self._encode_sample(np.arange(len(self._storage)))
...
class DynamicsReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        for i in idxes:
            ...
            obses_tp1.append(np.array(obs_tp1, copy=False))
            dones.append(done)
        return np.array(obses_t), np.array(actions), np.array(obses_tp1), np.array(dones)
