...
class BaseWorldModel(nn.Module):
    ...
    def __init__(self, x_dim, a_dim, vector_actions=False, device='cpu'):
        # type: (tuple, tuple, bool, str) -> None
        """"
        ...
        self.a_dim = a_dim
        self.vector_actions = vector_actions
        action_type_print = 'vector' if vector_actions else 'index'
        print('World model has input dimensions:', x_dim, '+', a_dim, '. Action type:', action_type_print)
    ...
    def apply_state_constraints(self, x, x_name=''):
        ...
        """
        x = self.apply_tensor_constraints(x, x_name)
    ...
    def apply_action_constraints(self, a, name='action'):
        ...
        """
        a = self.apply_tensor_constraints(a, name)
    ...
    def action_index_to_onehot(self, a_i):
        ...
        one_hot[batch_dim, a_i[:, 0].to(dtype=torch.long)] += 1  # Place ones into taken action indices
        return one_hot
...
class WorldModel_Recurrent(BaseWorldModel):

    def __init__(self, x_dim, a_dim, vector_actions=False, cell_dim=256, linear_dim=(256,), device='cpu'):
        # type: (tuple, tuple, bool, int, tuple, str) -> None
        """"
        A world model predicts the state at t+1, given state at t using a feed-forward model.
        This class predicts a distribution over latent space variables. Distributions are assumed Gaussian.
        """
...
class WorldModel_Recurrent_Sigma(BaseWorldModel):
