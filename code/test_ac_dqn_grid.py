...
def warmup(env, alg, wm, total_history):
    print('Warming up...')
    for _ in range(1):
        s_t = env.reset()
...
def main(env, visualise, folder_name, **kwargs):
    ...
    # warmup(env, alg, wm, total_history)
        ...
        while not done:
            ...
            if alg.train_steps < kwargs['train_steps']:
                ...
                if alg.train_steps % kwargs['interval'] == 0:
                    ep_scores['DQN'].append(np.mean(total_history['ext']))
                    ep_scores['Mean intrinsic reward'].append(np.mean(total_history['int']))
                    total_history = {'ext': [], 'int': []}
                    ...
                    if alg.train_steps % (kwargs['interval']*10) == 0:
                    ...
                    if env_name[:9] == 'GridWorld':
                        ...
                        visualise.eval_iteration_update(density_map=info['density'],
                                                        pe_map=pe_map,
                                                        ape_map=ape_map,
                                                        walls_map=walls_map)
...
if __name__ == "__main__":
    ...
    args = {'save_dir': 'results_grid/',
            'env_name': env_name,
            'name': 'eps01_nodone_wmonline_nodetach_tp1gradient_zdim32_act5',
